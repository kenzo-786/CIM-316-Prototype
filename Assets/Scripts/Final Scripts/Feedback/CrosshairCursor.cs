...
public class CrosshairCursor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        crosshairRoot.anchoredPosition = localPoint + offset;
    }

    private void UpdateCursorMode(bool force)
    {
        bool uiOpen = IsAnyUIScreenOpen();

        if (!force && uiOpen == usingUiCursor)
            return;

        usingUiCursor = uiOpen;

        if (crosshairRoot != null)
            crosshairRoot.gameObject.SetActive(!usingUiCursor);
        ...
        {
            Cursor.visible = !hideSystemCursorDuringGameplay;
            Cursor.lockState = CursorLockMode.None;
        }
    }
    ...
    {
        GameObject rootObject = new GameObject("Crosshair");
        rootObject.transform.SetParent(canvas.transform, false);

        crosshairRoot = rootObject.AddComponent<RectTransform>();
        ...
        CreateBar("Top", new Vector2(0f, gap + length * 0.5f), new Vector2(thickness, length));
        CreateBar("Bottom", new Vector2(0f, -gap - length * 0.5f), new Vector2(thickness, length));
        CreateBar("Right", new Vector2(gap + length * 0.5f, 0f), new Vector2(length, thickness));
        CreateBar("Left", new Vector2(-gap - length * 0.5f, 0f), new Vector2(length, thickness));
    }

    private void CreateBar(string objectName, Vector2 position, Vector2 size)
    {
        GameObject barObject = new GameObject(objectName);
        barObject.transform.SetParent(crosshairRoot, false);
